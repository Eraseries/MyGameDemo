...
namespace UnityGameFramework.Runtime
{
    ...
    public sealed partial class UIComponent : GameFrameworkComponent
    {
        ...
        private void Start()
        {
            ...
            int panel_depth = 1000;
            for (int i = 0; i < m_UIGroups.Length; i++)
            {
                if (!AddUIGroup(m_UIGroups[i].Name, m_UIGroups[i].Depth, panel_depth))
                {
                    Log.Warning("Add UI group '{0}' failure.", m_UIGroups[i].Name);
                    continue;
                }
                panel_depth = panel_depth - 100;
            }
        }
        ...
        public bool AddUIGroup(string uiGroupName,int panel_depth)
        {
            return AddUIGroup(uiGroupName, 0, panel_depth);
        }
        ...
        /// <param name="panel_depth">面板Z坐标。</param>
        /// <returns>是否增加界面组成功。</returns>
        public bool AddUIGroup(string uiGroupName, int depth,int panel_depth)
        {
            ...
            transform.localScale = Vector3.one;
            transform.localPosition = new Vector3(0, 0, panel_depth);
